...
def onlyfirst_n(inputfile,output_file):
        ...
        for row in dict_reader:
                ...
                count += 1
        print 'post_writed: ',post_writed
        print 'processed: ', count
        print 'post_cut',post_cut
        return 'done'
...
def find_data(db,input_file,output_file):
...
def count_accepted_answers(input_file):

        dict_reader = csv.DictReader(open(input_file, 'r'), delimiter=';') # DELIMITER
        tot_answers=0
	tot_answers_accepted=0
        for row in dict_reader:
             tot_answers+=1
	     #if(str(row['Accepted'])== 'yes'):
             if(str(row['Successful'])== 'yes'):
  
	   	tot_answers_accepted+=1
                        
        print 'Total answers', tot_answers
        print 'Total answers accepted', tot_answers_accepted
        return 'Done'
...
def onlymyanswersfilter1(db, input_file, output_file):
...
def onlymyanswersfilter2(db, input_file, output_file):
	...
	for row in dict_reader:
		...
		tot_tuples = long(result_set5.rowcount)
	...
	print 'Post processed: ', total
	print 'Risposte valide:', writed
	print 'Risposte tagliate: ', cut
	return 'Done'
...
def filterPredictor( input_file, output_file):
